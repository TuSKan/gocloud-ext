...
// BeforeList and BeforeDelete assert rather than returning nil. They were
// no-ops before, which is why nothing noticed that ListPaged never called
// BeforeList at all.
func (verifyAs) BeforeList(as func(any) bool) error {
	var c *sftp.Client
	if !as(&c) {
...
func (verifyAs) BeforeDelete(as func(any) bool) error {
	var c *sftp.Client
	if !as(&c) {
...
func TestOpenBucketURL(t *testing.T) {
	...
	tests := []struct {
		...
		// misspelled security setting must not fail open.
		{"sftp://host?knownhosts_path=/tmp/hosts", `invalid query parameter "knownhosts_path"`},
		{"sftp://host?insecure_skip_verify", `query parameter "insecure_skip_verify"`},

		// Booleans are parsed, so "false" means false. Before this was fixed,
		// "?insecure_skip_verify=false" disabled host key verification.
		{"sftp://host?insecure_skip_verify=nope", `query parameter "insecure_skip_verify"`},
		{"sftp://host?create_dir=maybe", `query parameter "create_dir"`},
		// The security assertion itself: "=false" must still consult
		// known_hosts, so reaching the known_hosts parse proves verification
		// was not skipped.
		{"sftp://host?insecure_skip_verify=false&known_hosts_path=/does/not/exist/hosts.txt", "failed to parse known_hosts file"},
